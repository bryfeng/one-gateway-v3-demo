...
- The credentialed Flow harness sends one test USDC from a freshly verified payer wallet to the EVM settlement address entered at runtime. It can create a real Dynamic Flow, wait for its risk decision, quote, request the payer’s wallet approval and record source and settlement hashes. The destination is locked in the local attempt once the Flow is created.
