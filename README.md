...
- No Dynamic API token has been created. Flow creation still requires a backend-held token with only `flow.write`; it must never be added to this Vite app, GitHub, or GitHub Pages.
...
- The development and deployed origins already pass the environment’s public CORS check. Remaining live setup is external-wallet sign-in verification, a distinct merchant test address, and the private Worker credentials.
